...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='esp-voicebot-tests-') as tmp:
        for name in ('test_audio_envelope', 'test_audio_pipeline', 'test_capture_pipeline', 'test_duplex_timeline', 'test_face_renderer', 'test_face_state', 'test_microphone_flow', 'test_playback_flow', 'test_session_flow', 'test_speaker_packetizer', 'test_websocket_stream', 'test_websocket_writable'):
        ...
        subprocess.run(['bash', str(sketch / 'tests/face_panel/run.sh')], check=True)
