...
All **15 host suites** passed AddressSanitizer and UndefinedBehaviorSanitizer. Regression coverage exercises the configured GPIO3 TFT through the shared guard into SPI initialization and drawing, rejects all eight active audio/button/status pins in each display role, and checks both permanently powered and GPIO-controlled backlights.

All three builds passed without compiler warnings using Arduino CLI 1.5.1, ESP32 core 3.3.11 and ArduinoJson 7.4.3 with dummy credentials:

| Build (AEC enabled) | Flash bytes | Static internal RAM bytes | RAM after globals |
| --- | ---: | ---: | ---: |
| Display enabled, OPI PSRAM | 1,191,079 | 83,316 | 244,364 |
| Display enabled, PSRAM disabled | 1,185,873 | 82,864 | 244,816 |
| Display disabled, OPI PSRAM | 1,179,439 | 82,268 | 245,412 |

Source manifests in `build/compile-f3auuoq6/` (display enabled, both PSRAM profiles) and `build/compile-znbdez9t/` (display disabled) match the firmware sources, apart from substituted test credentials. RAM after globals excludes runtime allocations. No ESP32 was connected, so the corrected firmware has not been flashed or observed on the physical panel.

### Display integration baseline recorded on 2026-09-18 (`1492a4e`)
