...
The face uses **320×240 landscape** (`VOICEBOT_DISPLAY_ROTATION 3`); use `1` for the opposite mounting direction. The default `VOICEBOT_DISPLAY_SOFTWARE_ROTATION 1` maps each rendered row into a column within the panel's native 240×320 address space, including transformed bounds for the full-screen clear. It uses the existing row buffers and needs no framebuffer. This avoids depending on the controller's axis-swap command, which did not correct the sideways face on the connected panel. A portrait override (`0` or `2`) produces a compile-time message. Set software rotation to `0` only to compare the controller rotation path.

The software-mapping build passed host tests and USB boot checks, but the user still reported a sideways, corrupted display. Physical panel behavior remains unresolved; see [the troubleshooting record](TROUBLESHOOTING_AND_FIXES.md#native-window-software-landscape-checked-on-2026-09-18). A successful rotation log does not verify the visible result.

If upgrading, update the display/button overrides in `config.local.h` as shown in [hardware_pinout.md](hardware_pinout.md#6-updating-an-existing-local-configuration), including **`VOICEBOT_BUTTON_PIN 46`**, **`VOICEBOT_DISPLAY_ROTATION 3`**, **`VOICEBOT_DISPLAY_SOFTWARE_ROTATION 1`** and **`VOICEBOT_DISPLAY_SPI_HZ 10000000`**. Existing local definitions take precedence over updated defaults. Microphone SCK and status LED come from `hardware_pins.h` and are now GPIO48 and GPIO13. Expect `[PINS] INMP441 SCK=48 WS=2 SD=1; BUTTON=46; STATUS_LED=13.` and `[FACE] ILI9341 320x240 at 10MHz; ...; landscape rotation=3; mapping=software/native240x320.`; a conflict log includes the exact GPIO.
...
USB boot monitoring has confirmed the new pins, PSRAM, AEC startup and Wi-Fi association. Visual orientation, colour order, voice-call behavior, Wi-Fi/TLS peak memory and a long-running soak still require physical checks; if the face appears upside down, change `VOICEBOT_DISPLAY_ROTATION` to 1. Backpressure preserves bounded RAM; a server may still end a session on a service or network error. A barge-in event behind already queued TCP audio cannot be processed until those preceding bytes are read. On receipt, queued playback is cancelled and subsequent audio waits in TCP until the in-flight frame/DMA guard and required playback notifications finish. No timed window of new TTS is discarded. Normal completion waits until the final software frame has entered I2S plus a conservative 120 ms DMA/acoustic drain guard; a post-audio bot-response event provides an end hint, with a 600 ms receive-idle fallback because the protocol has no explicit TTS audio-end marker. Any further audio invalidates the previous hint. In half-duplex fallback, listening resumes after completion is sent; AEC mode keeps listening throughout playback.
