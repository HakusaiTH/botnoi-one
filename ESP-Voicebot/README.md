...
Host suites exercise the actual framing, capture/speaker packetizers, duplex callback timeline, playback/session helpers, real socket congestion, and client logic with fake transport and real ArduinoJson, under AddressSanitizer and UndefinedBehaviorSanitizer. Wrapper doubles test AEC admission/cleanup policy; they do not establish echo-cancellation quality. See [TROUBLESHOOTING_AND_FIXES.md](TROUBLESHOOTING_AND_FIXES.md) for measured results and hardware checks.
